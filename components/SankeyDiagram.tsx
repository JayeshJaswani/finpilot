...
const getUnitMultiplier = (unit: string | undefined): number => {
  if (!unit) return 1;
  const lowerUnit = unit.toLowerCase();
  if (lowerUnit.includes('trillion')) return 1_000_000_000_000;
  if (lowerUnit.includes('billion')) return 1_000_000_000;
  if (lowerUnit.includes('million')) return 1_000_000;
  if (lowerUnit.includes('crore')) return 10_000_000;
  if (lowerUnit.includes('lakh')) return 100_000;
  if (lowerUnit.includes('thousand')) return 1_000;
  return 1;
};
...
export const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({ result, sources, onReset, fileName }) => {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-8 animate-fade-in relative">
      ...
      <ValuationCard
        historicalRevenue={financialsLocal.revenue * getUnitMultiplier(financialsLocal.unit)}
        companyName={companyName}
        currency={currency}
        unit={financialsLocal.unit}
        exchangeRate={financialsUSD?.exchangeRate && financialsUSD.exchangeRate > 2
          ? 1 / financialsUSD.exchangeRate
          : financialsUSD?.exchangeRate || 1.0}
      />
        ...
        <div className="w-full bg-gray-800 rounded-lg border border-gray-700 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            ...
            <a href="https://www.alphavantage.co/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-1 bg-gray-900/50 hover:bg-gray-900 rounded-full border border-gray-700 hover:border-blue-500/50 transition-colors group">
              ...
              <span className="text-sm font-bold text-blue-400">AlphaVantage</span>
            </a>
          </div>
